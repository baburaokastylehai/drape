...
const returnPattern = /(return started|return initiated|return received|return complete|drop off your return|return confirmation|return request|return approved|return label|your return|we('ve| have) received your return|exchange (confirmed|processed|approved|initiated)|return pickup|return picked|return scheduled|successfully returned|item.*returned|return created|return id|return.*accepted|reverse pickup)/i;
const refundPattern = /(refund processed|refund issued|refund complete|we refunded|amount refunded|your refund is on the way|refund confirmed|credit applied|credit issued|refund to your|money back|refund initiated|refund successful|amount credited|amount reversed|refund of)/i;
const promoPattern = /(sale|% off|extra off|shop now|new arrivals?|just dropped|lookbook|gift guide|exclusive offer|flash sale|ends tonight|limited time|member days|promo code|black friday|cyber monday|style edit|you might also like|recommended for you|complete the look|trending now|best sellers?|shop the collection|shop the look|editors? picks?|new season|last chance|final sale|back in stock|price drop|wish\s*list|saved items?|still interested|left something behind|cart reminder|don't forget|viewed recently|browse more|pick up where you left|items? you (liked|loved|viewed)|your (favorites?|picks))/i;
